...
public class ExclusiveIdempotentMethod {
     ...
     * (1) No other thread is currently running the wrapped method, in which case the current thread
     * will run the method at least once, then return true
     * (2) Another thread is already running the wrapped method, in which case that thread will run it at least
     * once the current thread immediately returns false;
     *
     * @return
     */
    public boolean tryRun() {
        ...
        if (_pending.getAndIncrement() == 0){// increment by 1; if previously 0, we now have the "lock"
            ...
            while (true){
                ...
                if (pending == 0){
                    // if we decrement and see 0, nobody else has called tryRun() since we started,
                    // and we can quit
                    return true;
                }else if(pending > 1){
